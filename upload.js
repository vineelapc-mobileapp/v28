...
fetch('data/questions.json')
  .then(r => r.json())
  .then(json => { DATA = json; populateSubjects(); });
...
publishBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!getRes.ok) {
      ...
      throw new Error(`Could not read current file (HTTP ${getRes.status}). Check username/repo/path/token. ${errBody.slice(0,200)}`);
    }
    ...
    if (!putRes.ok) {
      ...
      throw new Error(`Publish failed (HTTP ${putRes.status}). ${errBody.slice(0,200)}`);
    }
